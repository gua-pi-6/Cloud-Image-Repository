            ...
            <div class="art-card team-card">
              <div class="card-image-box">
                ...
                <div class="team-actions">
                  ...
                  <a-tooltip title="以图识图"><a-button @click="handleImageSearch(item)" shape="circle" class="icon-btn"><search-outlined /></a-button></a-tooltip>
...
const space = ref<API.SpaceVO>({
  id: 0, spaceName: '', spaceLevel: 0, maxSize: 1, maxCount: 0, totalSize: 0, totalCount: 0,
})
...
const handleDownload = (item: API.PictureVO) => {
  ...
  try { saveAs(item.url, item.name ? `${item.name}` : 'picture') } catch (e) { message.error('下载失败') }
}
...
const handleDelete = async (item: API.PictureVO) => {
  ...
  if (res.data.code === 0 && res.data.data) { message.success('删除成功'); fetchPictures() } else { message.error(res.data.message) }
}
...
const searchCondition = computed<API.PictureQueryRequest>(() => ({
  ...
  spaceId: props.id, nullSpaceId: true, startEditTime: searchParams.startEditTime, endEditTime: searchParams.endEditTime,
}))
...
const fetchSpaceVo = async () => {
  ...
  if (res.data.code === 0 && res.data.data) { space.value = res.data.data } else { message.error(res.data.message) }
}
const toCreationPage = async () => { router.push(`/creation/picture`) }

onMounted(async () => { await fetchSpaceVo(); fetchPictures() })

const storagePercent = computed(() => parseFloat((((space.value?.totalSize ?? 0) / (space.value?.maxSize ?? 1)) * 100).toFixed(2)))
...
#teamSpacePage { min-height: 100vh; padding-bottom: 40px; }
...
.team-panel {
  background: linear-gradient(145deg, #ffffff, #f0f5ff); position: relative; overflow: hidden;
  &::after {
    content: ''; position: absolute; bottom: -20px; right: -20px; width: 120px; height: 120px;
    background: radial-gradient(circle, rgba(47, 84, 235, 0.05) 0%, transparent 70%); border-radius: 50%;
  }
  .team-badge { display: inline-flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; color: $team-primary; background: rgba(47, 84, 235, 0.1); padding: 6px 12px; border-radius: 20px; margin-bottom: 16px; }
  .team-title { font-size: 22px; font-weight: 700; color: #1a1a1a; margin-bottom: 8px; }
  .team-desc { color: #666; font-size: 14px; margin-bottom: 0; }
  .panel-footer { margin-top: 24px; z-index: 1; }
  .action-btn { background: $team-primary; border: none; border-radius: 8px; font-weight: 500; display: flex; align-items: center; gap: 8px; transition: $transition-smooth; &:hover { background: #1d39c4; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(47, 84, 235, 0.3); } }
}

.stats-panel {
  justify-content: flex-start;
  .stats-header { display: flex; align-items: center; gap: 8px; margin-bottom: 20px; .stats-icon { font-size: 18px; &.text-blue { color: $team-primary; } &.text-green { color: #52c41a; } } h4 { margin: 0; font-size: 15px; font-weight: 600; color: #333; } }
  .stats-body { flex: 1; display: flex; }
}

.count-body { flex-direction: column; justify-content: center; align-items: center; text-align: center; background: #fafafa; border-radius: 12px; padding: 20px; .count-wrapper { margin-bottom: 8px; } .highlight-number { font-size: 36px; font-weight: 800; color: $team-primary; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; } .count-unit { font-size: 14px; color: #666; margin-left: 4px; font-weight: 500; } .stats-subtext { font-size: 13px; color: #999; margin: 0; } }
.storage-body { align-items: center; gap: 20px; .storage-info { flex: 1; display: flex; flex-direction: column; gap: 12px; } .stat-row { display: flex; justify-content: space-between; align-items: center; font-size: 13px; padding-bottom: 6px; border-bottom: 1px dashed #f0f0f0; &:last-child { border-bottom: none; padding-bottom: 0; } } .stat-label { color: #666; } .stat-value { color: #333; font-weight: 600; } }

.search-section { margin-bottom: 24px; }
.main-search-bar { padding: 16px 24px; margin-bottom: 0; .search-input-wrapper { display: flex; align-items: center; gap: 12px; } .search-icon { font-size: 20px; color: $team-primary; margin-right: 4px; } .main-search-input { flex: 1; border-radius: 8px; border: 1px solid #e8e8e8; &:hover, &:focus { border-color: $team-primary; box-shadow: 0 0 0 2px rgba(47, 84, 235, 0.1); } } .search-btn { border-radius: 8px; background: $team-primary; border: none; font-weight: 500; &:hover { background: #1d39c4; } } }
.filter-toggle-btn { display: flex; align-items: center; gap: 4px; color: #666; &:hover, .filter-active { color: $team-primary; } }
.filter-count { background: #ff4d4f; color: #fff; padding: 0 6px; border-radius: 10px; font-size: 12px; margin-left: 4px; }
.advanced-filter-panel { margin-top: 16px; background: #fff; border-top: 3px solid $team-primary; }
.filter-item { .filter-label { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; font-size: 14px; color: #333; font-weight: 500; .label-icon { color: $team-primary; } } :deep(.ant-select-selector), :deep(.ant-picker) { border-radius: 8px; } }
.quick-date-filters { margin-top: 16px; padding-top: 16px; border-top: 1px dashed #f0f0f0; display: flex; align-items: center; gap: 12px; .quick-label { font-size: 13px; color: #999; } .quick-tag { cursor: pointer; border-radius: 6px; padding: 4px 12px; border: 1px solid #e8e8e8; background: #fafafa; color: #666; transition: all 0.2s; &:hover { color: $team-primary; border-color: $team-primary; } &.active { background: $team-primary; border-color: $team-primary; color: #fff; } } }
.filter-actions { margin-top: 16px; padding-top: 16px; border-top: 1px dashed #f0f0f0; display: flex; justify-content: space-between; align-items: center; .reset-btn { border-radius: 6px; } .filter-summary .count-highlight { color: $team-primary; margin: 0 4px; } }

.team-grid { margin-top: 10px; }
.team-card { background: #fff; border-radius: 12px; overflow: hidden; cursor: pointer; transition: $transition-smooth; border: 1px solid #f0f0f0; &:hover { transform: translateY(-6px); box-shadow: $shadow-hover; border-color: rgba(47, 84, 235, 0.2); .card-image-box img { transform: scale(1.05); } .team-actions { opacity: 1; transform: translateY(0); } } }
...
.team-actions { position: absolute; bottom: 12px; right: 12px; display: flex; gap: 6px; opacity: 0; transform: translateY(10px); transition: $transition-smooth; }
.icon-btn { background: rgba(255, 255, 255, 0.9); border: none; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.1); color: #666; width: 32px; height: 32px; &:hover { color: $team-primary; background: #fff; transform: scale(1.05); } }
...
@media (max-width: 768px) { .dashboard-header .ant-col { margin-bottom: 16px; } .glass-panel { margin-bottom: 0; } .storage-body { flex-direction: column; text-align: center; .storage-info { width: 100%; } } .search-input-wrapper { flex-wrap: wrap; .main-search-input { min-width: 100%; margin-bottom: 8px; } .search-btn { width: 100%; } } }
</style>
